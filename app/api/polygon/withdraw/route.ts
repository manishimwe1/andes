...
import { isValidPolygonAddress, sendPolygonTransaction } from '@/lib/polygon/utils';
...
export async function POST(req: Request) {
    try {
        ...
        if (network !== 'polygon') {
             return NextResponse.json({ error: "Only Polygon withdrawals are supported via this endpoint" }, { status: 400 });
        }
        ...
        const type = tokenType || "POLYGON"; // Default to Native MATIC
        if (!["POLYGON", "USDT", "USDC"].includes(type)) {
        ...
        // 3. Process withdrawal on Polygon Blockchain
        try {
            console.log(`Processing ${type} withdrawal: ${amount} to ${address}`);

            // Execute Transfer
            const txHash = await sendPolygonTransaction(address, parseFloat(amount), type as any);
