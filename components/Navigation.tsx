...
const NavigationContent = () => {
    ...
    return (
        <nav suppressHydrationWarning className="fixed top-0 left-0 right-0 flex justify-between items-center px-[5%] py-6 bg-gradient-to-b from-white/95 to-white/85 backdrop-blur-lg z-50 shadow-md">
            <Link href="/" className="font-playfair text-3xl font-bold text-cyan-500 tracking-widest">
                ANDES
            </Link>
                ...
                <li>
                    {!session ? (
                        <div className="flex gap-4 items-center">
                            <Link href="/sign-in" className="text-sm text-gray-800 font-medium hover:text-cyan-500">
                                Sign In
                            </Link>
                            <Link href="/register" className="text-sm text-white font-medium bg-cyan-500 px-3 py-1 rounded hover:opacity-90">
                                Register
                            </Link>
                        </div>
                    ) : (
            ...
            <>
                ...
                {/* Mobile Menu */}
                {isOpen && (
                    <ul className="absolute top-20 left-0 right-0 flex flex-col bg-white/95 backdrop-blur-lg list-none md:hidden shadow-lg" suppressHydrationWarning>
                        {links.map((item, index) => (
                            <li key={index} className="border-b border-gray-200 last:border-b-0" suppressHydrationWarning>
                                <Link
                                    href={item.href}
                        ...
                        <li className="border-b border-gray-200">
                            {!session ? (
                                <div className="flex flex-col px-[5%] py-4 gap-3">
                                    <Link href="/sign-in" onClick={() => setIsOpen(false)} className="text-gray-800">Sign In</Link>
                                    <Link href="/register" onClick={() => setIsOpen(false)} className="text-white bg-cyan-500 px-3 py-2 rounded text-center">Register</Link>
                                </div>
                            ) : (
                                <button
                                    onClick={() => signOut({ redirect: true, callbackUrl: '/' })}
